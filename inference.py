...
def log_step(step_num, action, reward, done, error=None):
    ...
    print(f"[STEP] step={step_num} action={action} reward={reward} done={str(done).lower()} error={error_str}")
...
def run_episode(task_id="easy"):
    ...
    try:
        ...
        actions = ["hold", "hold", "hold", "buy", "hold", "hold", "sell", "hold", "hold"]
...
def main():
    ...
    # Summary
    print(f"[SUMMARY] completed={len(results)} tasks={','.join(t['task'] for t in results)}")
